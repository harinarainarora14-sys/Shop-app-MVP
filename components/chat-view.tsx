...
export function ChatView({ conversation, currentUserId, isCustomer }: ChatViewProps) {
  ...
  useEffect(() => {
    ...
    if (lastMessage.sender.id === currentUserId) {
      ...
      const timer = setTimeout(() => {
        sendMessage(
          "Our shop will get back to you soon. Meanwhile, you can check our inventory for updates.",
          true
        )
      }, 120000)
  ...
  const markAsAnswered = async (response: string) => {
    const lastCustomerMessage = [...messages].reverse().find((m) => m.sender.id !== currentUserId)

    if (lastCustomerMessage) {
      await supabase.from("messages").update({ is_answered: true }).eq("id", lastCustomerMessage.id)
